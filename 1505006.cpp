...
int main(int argc, char *argv[])
{
    ...
    while(true)
    {
        ...
        else if(action=="frwd")         ///SEMI TESTED
        {
            ...
            pointing=(char *)buff;
            strncpy(message,pointing,temp_cost);
            //printf("%s\n", message);
            string go,msgstr;
            go=string(router_1_address);
            msgstr=string(message,temp_cost);
            if(go==self_ip)
            {
        ...
        else if(action=="rout")
        {
            ...
            for(int i=0;i<neighbours.size();i++)
            {
            	if(neighbours[i].dest==receive_from)
            	{
            		cxv=neighbours[i].cost;
            		break;
            	}
            }
            for(int i=0;i<routing_table.size();i++)
            {
            	...
            	if(routing_table[i].nexthop==receive_from)
            	{
            			routing_table[i].cost=cxv+cost_array[x][y];
            	}
            	else
            	{
            		int tcst=10000;
            		...
            		{
            			if(routing_table[i].nexthop==neighbours[j].dest)
            			{
            				tcst=neighbours[j].cost;
            			}
            		}

            		if(cxv + cost_array[x][y] < routing_table[i].cost || tcst == 10000)
            		{
            			routing_table[i].cost=cxv+cost_array[x][y];
            			routing_table[i].nexthop=receive_from;
            		}
            	}
            }
            printf("--------------------------------\n");
